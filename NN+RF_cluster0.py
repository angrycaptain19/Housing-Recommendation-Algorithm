...
cluster0=pd.read_excel('listings_clusters1211.xlsx',sheet_name='聚类0')
print(cluster0['host_time'][0])



#定义自变量和因变量的名字
X=cluster0[["host_time",
...
#建立cluster0 的 multilayer neural network (unstandardized)
regr=MLPRegressor(hidden_layer_sizes=(240,250),activation='relu',
                  solver='adam',alpha=0.0001,batch_size='auto',
                  learning_rate_init=0.001,
                  max_iter=3000,shuffle=True,random_state=1210,
                  tol=1e-5,verbose=False,warm_start=False,early_stopping=False,
                  beta_1=0.9,beta_2=0.999,epsilon=1e-8,
                  n_iter_no_change=10)      
NN=regr.fit(x_train,y_train)
print('Cluster 0 Multilayer Neural Network (unstandardized)')
...
#建立cluster0 的 multilayer neural network (standardized)
regr_std=MLPRegressor(hidden_layer_sizes=(300,300),activation='logistic',
                  solver='adam',alpha=0.00001,batch_size='auto',
                  learning_rate_init=0.0005,learning_rate='adaptive',
                  max_iter=5000,shuffle=True,random_state=1210,
                  tol=1e-6,verbose=False,warm_start=False,early_stopping=True,
                  beta_1=0.9,beta_2=0.999,epsilon=1e-8,
                  n_iter_no_change=10)  
NN_std=regr_std.fit(x_train_std,y_train_std)
print('Cluster 0 Multilayer Neural Network (standardized)')
print('Train Score:  {}'.format(str(NN_std.score(x_train_std,y_train_std))))
print('Valid R^2:  {}'.format(str(r2_score(y_valid_std,NN_std.predict(x_valid_std)))))
print('Params:  {}'.format(str(NN_std.get_params)))
print('\n\n')
...
#对cluster0 的 multilayer neural network (standardized)调参建模
params_test1={
...
#对cluster0建立随机森林模型
regr_RF=RandomForestRegressor(n_estimators=950,criterion='mse',
                        max_depth=3,min_samples_split=2,
                         min_samples_leaf=1,min_weight_fraction_leaf=0.0,
                         max_features='auto',max_leaf_nodes=None,min_impurity_decrease=0.0,
                         min_impurity_split=None,bootstrap=False,n_jobs=5,verbose=2,warm_start=False)
RF=regr_RF.fit(x_train,y_train)
print('Cluster 0 Random Forest')
...
print('Cluster 0')
print('RF_tailor Best Score:')
print(best_score)
print('RF_tailor Best Params:')
print(best_params)
