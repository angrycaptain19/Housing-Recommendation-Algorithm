...
cluster2=pd.read_excel('listings_clusters1211.xlsx',sheet_name='聚类2')
print(cluster2['host_time'][0])


#定义自变量和因变量的名字
X=cluster2[["host_time",
...
#建立cluster2 的 multilayer neural network (unstandardized)
...
print('Cluster 2 Multilayer Neural Network (unstandardized)')
...
#建立cluster2 的 multilayer neural network (standardized)
regr_std=MLPRegressor(hidden_layer_sizes=(500,500),activation='logistic',
                  solver='adam',alpha=0.00001,batch_size='auto',
...
print('Cluster 2 Multilayer Neural Network (standardized)')
print('Train Score:')
...
#对cluster2 的 multilayer neural network (standardized)调参建模
params_test1={
    'hidden_layer_sizes':[(3000,2000),(1500,),(800,),(700,700)],
...
regr_std_tailor=MLPRegressor(batch_size='auto',shuffle=True,
                         max_iter=6000,verbose=True,warm_start=False)
...
#对cluster2建立随机森林模型
regr_RF=RandomForestRegressor(n_estimators=700,criterion='mae',
...
importances_list=list(zip(cluster2.columns[2:],feature_importances))
importances_list.sort(key=lambda x:x[1],reverse=True)
importances_sort=dict(importances_list)
print(importances_sort)
print('\n\n')
...
#对cluster2 的 随机森林调参建模
params_test2={
    'criterion':['mse','mse'],
    'max_depth':range(6,35,2),
    'min_samples_split':[2,3,4],
    'min_samples_leaf':[2,3],
    'max_features':range(10,30,2),
    'n_estimators':[100,125,150,175,200,250,300],
    }
regr_RF_tailor=RandomForestRegressor(min_weight_fraction_leaf=0.0,
...
feature_importances_CV_list=list(zip(cluster2.columns[2:],feature_importances_CV))
